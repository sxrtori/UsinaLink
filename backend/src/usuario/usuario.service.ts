...
export class UsuarioService {
  ...
  async cadastrar(dto:CreateUsuarioDto,tipoUsuario:TipoLogin){
    ...
    } else if(cnpj.length!==14) throw new BadRequestException('CNPJ invalido.');
    ...
    return this.ds.transaction(async m=>{
      ...
      if(tipoUsuario==='pessoa_fisica') return { message:'Cadastro realizado com sucesso.', usuario:this.safe(usuario) };
      const base={cnpj,email,telefone,razaoSocial:(dto as any).razaoSocial||(dto as any).nome,nomeFantasia:(dto as any).nomeFantasia||(dto as any).nome,responsavel:(dto as any).responsavel};
  ...
  async loginArea(dto:LoginDto, area:TipoLogin){
    ...
    if(!await this.passwords.compare(String(dto.senha||''), usuario.senhaHash)) throw new UnauthorizedException('E-mail ou senha invalidos.');
